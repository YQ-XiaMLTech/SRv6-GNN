...
class PPOAgent(object):
    ...
    def run_episode(self):
        ...
        #
        # print('run_episode,1',f"The code took {elapsed_time} seconds to run.")
        ...
        for t in range(self.horizon):
            ...
            # end_time = time.time()
            # elapsed_time = end_time - start_time
            # print('run_episodehorizon',t,f"The code took {elapsed_time} seconds to run.")
        value = self.run_critic(state)
    ...
    def train_and_evaluate(self):
        ...
        while not (self.env.num_sample == self.last_training_sample and self.change_sample == True):
            ...
            # elapsed_time = end_time - start_time
            #
            # print('train_and_evaluate',f"The code took {elapsed_time} seconds to run.")
    ...
    def training_eval(self):
        ...
        for eval_env_type in self.eval_env_type:
            ...
            for _ in range(self.num_eval_samples):
                ...
                probs, values = [], [],
                # print("good if,open for",_)
                list_name = f"reward_{_}"
                LU_max_list[list_name] = []
                for i in range(self.horizon):
                    ...
                    LU_max = self.eval_envs[eval_env_type].step_max_LU(action.numpy())
                    probs.append(np.exp(log_prob))
                    values.append(value.numpy()[0])
                    state = next_state
                    # rewards.append(reward)
                    # reward_lists[list_name].append(reward)
                    LU_max_list[list_name].append(LU_max)
                    if self.eval_envs[eval_env_type].link_traffic_to_states:
                    ...
                    # elapsed_time = end_time - start_time
                    #
                    # print('training_eval',eval_env_type,i,f"The code took {elapsed_time} seconds to run.")
                    # print(i,"over for")
                # print("rewards",eval_env_type,": ",rewards)
                ...
                mini_eval_episode += 1
                # print("reward_lists",[list_name],":",reward_lists[list_name])
            # rewards = [sum(sum_list) for sum_list in zip(*reward_lists.values())]
            LU_max_values = [sum(sum_list) for sum_list in zip(*LU_max_list.values())]

            print("LU_max_values", eval_env_type, LU_max_values)
            print(eval_env_type, "good")

            tf_logs.eval_top_log(self.writer, self.eval_episode, total_min_max, eval_env_type)
            tf_logs.eval_env(self.writer, self.eval_envs[eval_env_type], self.eval_episode, LU_max_values)

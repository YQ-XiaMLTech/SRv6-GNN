...
def eval_env(writer, env, eval_episode, LU_max_values):
    network = ('+').join(env.env_type)
    with writer.as_default():
        with tf.name_scope('Eval'):

            for i, reward in enumerate(LU_max_values):
                tf.summary.scalar(network + f" - LU_max_value {i}", reward, step=eval_episode)
